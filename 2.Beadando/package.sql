CREATE OR REPLACE PACKAGE BODY MENTO AS
    PROCEDURE RANDOM_AUTO(num INT) IS
    betuk VARCHAR2(6);
  szamok VARCHAR2(6);
...
EXCEPTION
    ...
    PROCEDURE UJ_HIVAS(Nev VARCHAR2,Telefonszam VARCHAR2,Hely VARCHAR2,Diszpecser VARCHAR2,Kikuldve INT)IS 
    ID INT;
DATUM DATE;
BEGIN
...
EXCEPTION
    ...
    FUNCTION NAPI_HIVASSZAM(PDATUM DATE) RETURN INTEGER IS
    SZAM INT;
BEGIN
    ...
    PROCEDURE KIVONULAS_FELTOLTES(szam INT) IS
    CSID INT;
AID INT;
BID INT;
KID INT :=1;
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
    END KIVONULAS_FELTOLTES;
    
    
    PROCEDURE HIVAS_ADMINISZTRALASA(PBID INT, Kimente NUMBER) IS
    BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
    END HIVAS_ADMINISZTRALASA;
    
    
    PROCEDURE HIVAS_NEVRE(PNEV VARCHAR2,CURSOR OUT SYS_REFCURSOR) IS
...
EXCEPTION
    ...
    PROCEDURE HIVAS_IDOSZAK(MIN DATE,MAX DATE,CURSOR OUT SYS_REFCURSOR) IS 
    BEGIN
